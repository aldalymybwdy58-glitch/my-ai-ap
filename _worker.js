...
async function chat(request, env) {
  ...
  try {
    ...
    if (!reply) {
      return json(
        {
          error: "النموذج عمل، لكن لم يرجع نصًا.",
          result: result,
        },
        500
      );
    ...
    return json(
      {
        error: "فشل تشغيل نموذج Workers AI.",
...
async function generateImage(request, env) {
  ...
  if (!prompt) {
    return json({ error: "اكتب وصف الصورة." }, 400);
  }

  if (!env.AI) {
    return json({ error: "Workers AI غير مربوط." }, 500);
  }
...
export default {
  async fetch(request, env) {
    ...
    return new Response(
      "ملفات التطبيق غير مربوطة بـ ASSETS.",
      {
        status: 503,
        headers: CORS,
      }
